...
for link in all_links:
    ...
    try:
        ...
        # LAY QUOC TICH
        try:
            # 1. Nếu có trường Nationality trong infobox → lấy luôn
            nationality_element = driver.find_element(By.XPATH, "//th[contains(text(), 'Nationality')]/following-sibling::td")
            nationality = nationality_element.text.strip()

        except:
            # 2. Nếu KHÔNG có mục Nationality → SUY LUẬN từ Birth place
            try:
                birth_element = driver.find_element(By.XPATH, "//th[contains(text(),'Born')]/following-sibling::td")
